...
function update()
  local button = brake
  local brake = not button
  local boost = not button

  buffered_accel = buffered_accel * 0.85 + 0.06 * 0.15
  buffered_turn = buffered_turn * 0.85 + turn * 0.15
  self.transform.facing =
    v2.norm(v2.rotate(self.transform.facing, buffered_turn / 10))

  local current_accel = buffered_accel * 0.06
  if boost then current_accel = current_accel + 0.06 end
